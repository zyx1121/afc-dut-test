# ref: https://github.com/Wi-FiTestSuite/AFC-DUT/blob/main/AFC-System-Simulator/afc_simulator_service/app/test_vectors/test-vectors-to-json.py
            ...
            InquiryResponse["availableChannelInfo"].append(
                {
                    "channelCfi": [
                        int(item)
                        for idx, item in enumerate(channelCfi)
                        if item
                        and item != "NULL"
                        and len(maxEirp) > idx
                        and maxEirp[idx]
                        and maxEirp[idx] != "NULL"
                    ],
                    "globalOperatingClass": data[i],
                    "maxEirp": [
                        float(item) for item in maxEirp if item and item != "NULL"
                    ],
                }
            )
    # vec["row"] = row # debug
